...
@pytest.mark.asyncio
async def test_a_carried_point_keeps_its_place_in_config_order():
    """Injection points are honoured in config order, earlier ones winning scarce slots.

    A carried point is placed a pass later than the ones that matched immediately, so
    without a reserved slot the earlier point in the config loses the cap to later ones
    purely because its target had not been built yet.
    """
    input_items: list = []
    for i in range(4):
        input_items.append({"role": "user", "content": [{"type": "input_text", "text": f"msg{i}"}]})
        input_items.append(
    ...
    body = await _bridged_body(
        mock_post,
        points=[{"location": "message", "role": "system"}, {"location": "message", "role": "user"}],
        input=input_items,
    )
